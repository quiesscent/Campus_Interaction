...
def search_building(request):
    ...
    if query:
        ...
        data = [{"name": b.name, "latitude": b.latitude, "longitude": b.longitude, "description": b.description} for b in buildings]
...
class LandmarksNearby(View):
    def get(self, request):
        ...
        for lm in Landmark.objects.all():
            distance = haversine(latitude, longitude, float(lm.latitude), float(lm.longitude))
            if distance <= radius:
                landmarks.append({
                    "name": lm.name,
