    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('HX-Request'):
                ...
                return render(request, 'feeds/partials/post_list.html', {'posts': [post]})
...
def post_detail(request, post_id):
    post = get_object_or_404(Post.objects.select_related('user')
                            .prefetch_related('comments__user'), id=post_id)
...
def load_comments(request, post_id):
    ...
    comments = post.comments.filter(parent=None).select_related(
        'user',
        'user__profile'  # Add this to prefetch profile data
    ).prefetch_related('replies')
    
    html = render_to_string('feeds/partials/comment.html', {
        'comments': comments,
        'user': request.user
    })
